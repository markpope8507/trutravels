...
| `partner-logos.html` | **Partner logos** — the white-tile logo wall and the testimonial carousel. White tiles aren't a style choice: two of the six marks are solid black and every one is a different colour, so nothing else works for all six without altering someone's logo. Logos carry explicit `width`/`height` or the lazy images lay out at zero and reflow the grid. Quotes not yet collected show an "awaiting copy" tag rather than invented words under a real org's mark. | yes |
...
| File | Component | Needs a `<script>`? |
|------|-----------|:---:|
| `choice-cards.html` | **Choice cards** — the three-up "pick the one that sounds like you" block, for a page whose job is to route people rather than sell one thing. Icon / title / one paragraph / CTA, with the CTA pinned to the card foot so all three line up however long the copy runs. A card with nowhere to go is a `<div>`, never an empty `<a>`. | no |
| `steps-rail.html` | **Steps rail** — the numbered how-it-works sequence. Numbers rather than icons *because the order carries information*; if the items read in any order, use `benefit-cards.html` instead. CSS counter, so steps can be added or removed without renumbering, and the connector line is on `:not(:last-child)` so the last step never trails into space. | no |
| `benefit-cards.html` | **Benefit cards** — the unordered "what you'll receive" grid. Icons, not numbers, because these read in any order. Five in a two-up grid would leave the last alone in a half row, so an odd last child spans full width. Shares `.job__win` with the job listing — same object, one definition. | no |
