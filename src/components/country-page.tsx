...
export default function CountryPage({ country }: { country: Country }) {
  ...
  return (
    <div className="relative overflow-x-clip">
      ...
      {/* Fun Facts */}
      {/* Straight into the trips, the same as the region page. The facts strip
          that used to sit here (language / currency / national dish / local
          beer) was dropped: four emoji boxes that earned little, and its
          negative margin covered the breadcrumbs. `country.facts` is still in
          the data if it's ever wanted back. */}
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 mb-20">
        ...
        <FaqAccordion faqs={country.faqs.map((f) => ({ q: f.question, a: f.answer }))} />
      </section>

      {/* You Might Also Like */}
      <DestinationsCarousel excludeName={country.name} />
      </div>
    </div>
  );
}
