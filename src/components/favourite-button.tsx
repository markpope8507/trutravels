...
export default function FavouriteButton({
  ...
  tripId: string;
  /** Included in the accessible label so hearts are distinguishable in a card list. */
  tripTitle?: string;
  /** "overlay" renders the contrast chip used when sitting on top of a card image. */
  ...
  // Cards put this button inside (or on top of) a <Link>; never let the toggle
  ...
  const label = tripTitle
    ? saved
      ? `Remove ${tripTitle} from saved`
      : `Save ${tripTitle}`
    : saved
  ...
  return (
    <div className="relative inline-block" ref={ref}>
      <button
        ...
        className={
          overlay
            ? "group flex h-8 w-8 items-center justify-center rounded-full bg-black/50 backdrop-blur-sm transition-colors duration-200 hover:bg-black/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-tru-pink"
            : "p-1 transition-colors duration-200 group"
        }
        aria-label={label}
        aria-pressed={saved}
      >
        {saved ? (
          ...
          <svg className={`h-5 w-5 group-hover:text-tru-pink ${overlay ? "text-white" : "text-gray-400"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
