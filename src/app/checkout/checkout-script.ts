...
export function runCheckout() {
    ...
    var IC = {
      ...
      info: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><path stroke-linecap="round" d="M12 11.25v4.75"/><circle cx="12" cy="7.9" r="1" fill="currentColor" stroke="none"/></svg>'
    };
    ...
    var cart = [], storedCart = null;
    try { storedCart = sessionStorage.getItem('truCart'); cart = JSON.parse(storedCart || '[]'); } catch (e) { cart = []; }
    if (!Array.isArray(cart)) cart = [];
    /* Seed the demo basket on a first visit only. Once every tour has been removed we
       leave it empty, so the no-trips view survives a reload instead of re-seeding. */
    if (storedCart === null && cart.length === 0) {
      cart = [
    ...
    var ADDONS = [
      ...
      /* `info` puts an (i) beside the name — hover, focus or tap for the caveat that a
         transfer outside the tour dates depends on a pre / post-night booked with us. */
      { id: 'arrival', cat: 'Transfers', name: 'Arrival Transfer', price: 60, icon: 'van', on: 'start', shift: 0, ctx: function (t) { return 'Pick-up from ' + t.startLocation + ' Airport'; }, info: function (t) { return 'If you are arriving before your tour start date you can still be picked up from ' + t.startLocation + ' Airport, however you will need to book a pre-night(s) accommodation with us so we have somewhere to take you. If you are arriving on your tour start date, please skip this.'; } },
      { id: 'departure', cat: 'Transfers', name: 'Departure Transfer', price: 60, icon: 'van', on: 'end', shift: 1, ctx: function (t) { return 'Drop-off to ' + t.endLocation + ' Airport'; }, info: function (t) { return 'If you are leaving after your tour end date you can still be transferred to ' + t.endLocation + ' Airport, however you will need to book a post-night(s) accommodation with us. If you are leaving on your tour end date, please skip this.'; } }
    ];
    ...
    /* Pre / post-night hotel is configurable: N nights × how the group is rooming.
       `sleeps` is how many of the booking's travellers one unit covers, which is what
       caps the panel — a party of 2 can never be given 3 beds. `min2` options only
       appear once 2+ people are travelling. All four are listed so the build team can
       see every rooming option the page has to support. */
    var PN_NIGHTS = [1, 14];
    var PN_ROOMS = [
    ...
    var PN_FROM = PN_ROOMS.reduce(function (m, r) { return Math.min(m, r.price); }, Infinity);
    function isHotelAddon(id) { return id === 'prenight' || id === 'postnight'; }
    function pnRoom(id) { var f = null; PN_ROOMS.forEach(function (r) { if (r.id === id) f = r; }); return f; }
    function pnPlaced(c) { return PN_ROOMS.reduce(function (n, r) { return n + (c[r.id] || 0) * r.sleeps; }, 0); }
    function nightsCost(c) { return c && typeof c === 'object' ? c.nights * PN_ROOMS.reduce(function (s, r) { return s + (c[r.id] || 0) * r.price; }, 0) : 0; }
    function addonCost(a, t) { return isHotelAddon(a.id) ? nightsCost(t.addons && t.addons[a.id]) : a.price; }
    /* Opening position when the add-on is switched on: everyone in twin share. */
    function pnDefault(pax) { var c = { nights: 1 }; PN_ROOMS.forEach(function (r) { c[r.id] = 0; }); c.twin = Math.max(1, pax); return c; }
    /* Keep rooming inside the party size. If travellers are removed after rooms were
       picked, hand back the last-listed types first so the cheapest choice survives. */
    function pnClamp(c, pax) {
      c.nights = Math.max(PN_NIGHTS[0], Math.min(PN_NIGHTS[1], c.nights || 1));
      PN_ROOMS.forEach(function (r) { c[r.id] = Math.max(0, c[r.id] || 0); });
      for (var i = PN_ROOMS.length - 1; i >= 0; i--) {
        while (c[PN_ROOMS[i].id] > 0 && pnPlaced(c) > pax) c[PN_ROOMS[i].id]--;
      }
      return c;
    }
    function pnSync(t) {
      var pax = t.travellers || 1;
      ['prenight', 'postnight'].forEach(function (id) {
        var c = t.addons && t.addons[id];
        if (c) t.addons[id] = pnClamp(typeof c === 'object' ? c : pnDefault(pax), pax);
      });
    }
    /* Pre-night guests check in ahead of the start; post-night on the tour end date. */
    function hotelCheckin(a, t) { var c = t.addons && t.addons[a.id]; return a.id === 'prenight' ? shiftDate(t.date, -((c && c.nights) || 1)) : shiftDate(t.endDate || t.date, 0); }
    cart.forEach(pnSync);   /* a cart restored from an earlier session may pre-date these options */
    ...
    function totals() {
      ...
      var addonsTotal = 0;
      cart.forEach(function (t) { if (t.addons) ADDONS.forEach(function (a) { if (t.addons[a.id]) addonsTotal += addonCost(a, t); }); });
    ...
    function renderTrips() {
      var html = cart.map(function (t, i) {
        ...
        var addonSum = 0;
        if (t.addons) ADDONS.forEach(function (a) { if (t.addons[a.id]) addonSum += addonCost(a, t); });
        ...
        var addonCostRows = '';
        if (t.addons) ADDONS.forEach(function (a) { if (t.addons[a.id]) { var ac = addonCost(a, t); if (ac > 0) addonCostRows += '<div class="co-trip__costrow"><span>' + a.name + (isHotelAddon(a.id) ? ' &middot; ' + t.addons[a.id].nights + ' night' + (t.addons[a.id].nights > 1 ? 's' : '') : '') + '</span><span>+' + fmt(ac) + '</span></div>'; } });
    ...
    /* Pre / post-night hotels expand after adding — pick nights, then room the group. */
    function pnStepper(addonId, key, val, canDec, canInc) {
      return '<span class="co-pn__step">'
        + '<button type="button" class="co-pn__btn" data-pn-step="' + key + '" data-pn-addon="' + addonId + '" data-pn-dir="-1" aria-label="One fewer"' + (canDec ? '' : ' disabled') + '>&minus;</button>'
        + '<span class="co-pn__val">' + val + '</span>'
        + '<button type="button" class="co-pn__btn" data-pn-step="' + key + '" data-pn-addon="' + addonId + '" data-pn-dir="1" aria-label="One more"' + (canInc ? '' : ' disabled') + '>+</button>'
        + '</span>';
    }
    function hotelConfig(a, tr) {
      var c = tr.addons[a.id], pax = tr.travellers || 1;
      var pre = a.id === 'prenight';
      var hotel = 'TruTravels ' + (pre ? tr.startLocation : tr.endLocation) + ' Hotel';
      var cost = nightsCost(c), placed = pnPlaced(c), left = pax - placed;
      var lede = pre
        ? 'Arriving before your tour starts? Add the night(s) you need at our partner hotel so you\'re rested and ready for day one.'
        : 'Leaving after your tour ends? We can still get you to the airport — just add the post-night(s) you need. Leaving on your end date? You can skip this.';
      /* One row per room type, its + disabled as soon as the party is fully placed. */
      var rows = PN_ROOMS.filter(function (r) { return !r.min2 || pax > 1; }).map(function (r) {
        var n = c[r.id] || 0;
        return '<div class="co-pn__row' + (n ? ' is-picked' : '') + '">'
          + pnStepper(a.id, r.id, n, n > 0, left >= r.sleeps)
          + '<span class="co-pn__lbl"><span class="co-pn__name">' + r.name + '<span class="co-pn__rate">£' + r.price + ' ' + r.rate + '</span></span>'
          + '<small class="co-pn__note">' + r.note + '</small></span>'
          + '</div>';
      }).join('');
      var msg = placed === 0
        ? 'Choose how your group is rooming to include this stay.'
        : (left > 0 ? left + ' traveller' + (left === 1 ? '' : 's') + ' still ' + (left === 1 ? 'needs' : 'need') + ' a bed.' : '');
      return '<div class="co-pn__cfg">'
        + '<p class="co-pn__lede">' + lede + '</p>'
        + '<p class="co-pn__h">' + (pre ? 'Pre-tour accommodation' : 'Post-tour accommodation') + '</p>'
        + '<div class="co-pn__row">' + pnStepper(a.id, 'nights', c.nights, c.nights > PN_NIGHTS[0], c.nights < PN_NIGHTS[1]) + '<span class="co-pn__lbl"><span class="co-pn__name">Nights at <strong>' + hotel + '</strong></span></span></div>'
        + '<p class="co-pn__checkin">' + IC.cal + 'Check-in <strong>' + hotelCheckin(a, tr) + '</strong></p>'
        + '<div class="co-pn__rhead"><p class="co-pn__h">Rooming</p>'
        + '<p class="co-pn__cap' + (left > 0 ? ' is-warn' : '') + '">' + placed + ' of ' + pax + ' traveller' + (pax === 1 ? '' : 's') + ' placed</p></div>'
        + rows
        + (cost > 0 || msg
            ? '<div class="co-pn__foot">'
              + (cost > 0 ? '<div class="co-pn__total"><span>' + c.nights + ' night' + (c.nights > 1 ? 's' : '') + ' &middot; ' + placed + ' traveller' + (placed === 1 ? '' : 's') + '</span><strong>+' + fmt(cost) + '</strong></div>' : '')
              + (msg ? '<p class="co-pn__hint">' + msg + '</p>' : '')
              + '</div>'
            : '')
        + '</div>';
    }
    function addonRow(a, tr, idx) {
      ...
      var isHotel = isHotelAddon(a.id);
      var priceTxt = isHotel ? (on && addonCost(a, tr) > 0 ? '+' + fmt(addonCost(a, tr)) : 'from £' + PN_FROM) : '+' + fmt(a.price);
      var iconColor = a.cat === 'Accommodation' ? 'var(--tru-pink)' : 'var(--tru-blue)';
      /* Optional (i) beside the name — shown on hover/focus by CSS, on tap by the click handler */
      var infoHtml = a.info
        ? '<span class="co-info"><button type="button" class="co-info__btn" data-info aria-label="More about the ' + a.name + '" aria-expanded="false">' + IC.info + '</button><span class="co-info__pop" role="tooltip">' + a.info(tr) + '</span></span>'
        : '';
      var row = '<div class="co-addon ' + (on ? 'is-on' : '') + '">'
        + '<span class="co-addon__ico" style="color:' + iconColor + '">' + IC[a.icon] + '</span>'
        + '<div class="co-addon__info"><p class="co-addon__name">' + a.name + infoHtml + '</p><p class="co-addon__desc">' + a.ctx(tr) + '</p></div>'
        + '<div class="co-addon__right"><span class="co-addon__price">' + priceTxt + '</span><button type="button" class="co-addon__btn ' + (on ? 'is-on' : '') + '" data-addon-toggle="' + idx + ':' + a.id + '"' + (on ? ' aria-label="Added — tap to remove"' : '') + '>' + (on ? '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>' : 'Add') + '</button></div>'
        + '</div>';
      if (isHotel) return '<div class="co-pn' + (on ? ' is-on' : '') + '">' + row + (on ? hotelConfig(a, tr) : '') + '</div>';
      return row;
    }
    ...
    /* One tour "summary item" — reused by the desktop aside AND the full-width booking
       summary. Every tour is removable, including the last one. */
    function oiHtml(tr, i) {
      ...
      if (tr.addons) ADDONS.forEach(function (a) { if (tr.addons[a.id]) { var ac = addonCost(a, tr); if (ac > 0) { var sub = isHotelAddon(a.id) ? (tr.addons[a.id].nights + ' night' + (tr.addons[a.id].nights > 1 ? 's' : '') + ' · check-in ' + hotelCheckin(a, tr)) : addonDate(a, tr); ex += '<div class="co-oi__xline"><span class="co-oi__xname">' + IC[a.icon] + '<span class="co-oi__xtext"><span>' + a.name + '</span><small class="co-oi__xdate">' + sub + '</small></span></span><span class="co-oi__xprice">+' + fmt(ac) + '</span></div>'; } } });
      var days = parseInt(tr.duration, 10) || 1;
      var perDay = Math.round(per / days);
      return '<div class="co-oi">'
        + '<div class="co-oi__img"><img src="' + tr.image + '" alt="" onerror="this.style.display=\'none\'" />'
        + '<button type="button" class="co-oi__remove" data-remove-tour="' + i + '" aria-label="Remove ' + tr.tripTitle + ' from your booking" title="Remove tour"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button>'
        + '</div>'
    ...
    function renderBooking() {
      ...
      var tripHtml = cart.map(function (tr) {
        ...
        if (tr.addons) ADDONS.forEach(function (a) { if (tr.addons[a.id]) { var ac = addonCost(a, tr); if (ac > 0) ad += '<li>' + a.name + ' &middot; ' + (isHotelAddon(a.id) ? tr.addons[a.id].nights + ' night' + (tr.addons[a.id].nights > 1 ? 's' : '') : addonDate(a, tr)) + '<span>+' + fmt(ac) + '</span></li>'; } });
    ...
    document.addEventListener('click', function (e) {
      ...
      if (nq === q) return;
      /* same group on every tour — and rooming can't outlive the travellers it was for */
      cart.forEach(function (t) { t.travellers = nq; pnSync(t); });
      persist();
      renderAll();
    });

    /* ---- Remove a tour from the booking summary. Removing the last one is allowed —
       the booking then falls back to the no-trips view. ---- */
    document.addEventListener('click', function (e) {
      var rm = e.target.closest('[data-remove-tour]');
      if (!rm) return;
      var i = +rm.dataset.removeTour;
    ...
    /* With nothing in the booking there's no flow to render — swap in the no-trips view
       and stop, since every renderer below assumes at least one tour. */
    function renderEmpty() {
      var isEmpty = cart.length === 0;
      var empty = document.querySelector('[data-empty]');
      var grid = document.querySelector('[data-grid]');
      var prog = document.querySelector('.co-progress');
      if (empty) empty.hidden = !isEmpty;
      if (grid) grid.hidden = isEmpty;
      if (prog) prog.hidden = isEmpty;
      return isEmpty;
    }
    function renderAll() {
      if (renderEmpty()) return;
      renderPax(); renderTravellers(); renderTrips(); renderSummary(); renderCredit();
    }
    renderAll();
    ...
    /* ---- (i) popups. CSS handles hover/focus; this adds tap-to-open for touch. ---- */
    function closeInfo() {
      document.querySelectorAll('.co-info.is-open').forEach(function (w) {
        w.classList.remove('is-open');
        var b = w.querySelector('[data-info]'); if (b) b.setAttribute('aria-expanded', 'false');
      });
    }
    /* One document-level handler so every (i) behaves the same wherever it sits */
    document.addEventListener('click', function (e) {
      if (e.target.closest('.co-info__pop')) return;      /* clicks inside the bubble leave it open */
      var ib = e.target.closest('[data-info]');
      if (!ib) { closeInfo(); return; }
      var wrap = ib.parentNode, wasOpen = wrap.classList.contains('is-open');
      closeInfo();
      if (!wasOpen) { wrap.classList.add('is-open'); ib.setAttribute('aria-expanded', 'true'); }
    });
    ...
    document.querySelector('[data-trip-list]').addEventListener('click', function (e) {
      /* Info popups are handled globally — nothing here should read them as cart input */
      if (e.target.closest('[data-info]') || e.target.closest('.co-info__pop')) return;
      ...
      /* Pre / post-night nights + rooming steppers */
      var pn = e.target.closest('[data-pn-step]');
      if (pn) {
        var c = it.addons && it.addons[pn.dataset.pnAddon];
        if (c) {
          var pk = pn.dataset.pnStep, pdir = +pn.dataset.pnDir;
          if (pk === 'nights') {
            c.nights = Math.max(PN_NIGHTS[0], Math.min(PN_NIGHTS[1], c.nights + pdir));
          } else {
            /* never place more travellers in beds than are on the booking */
            var pr = pnRoom(pk);
            if (pr && (pdir < 0 ? c[pk] > 0 : pnPlaced(c) + pr.sleeps <= (it.travellers || 1))) c[pk] += pdir;
          }
      ...
      if (addon) {
        ...
        it.addons = it.addons || {};
        if (isHotelAddon(id)) {
          /* seed a sensible default (1 night, whole party in twin share) so the panel is useful on open */
          it.addons[id] = it.addons[id] ? false : pnDefault(it.travellers || 1);
        } else {
    ...
    document.querySelectorAll('[data-mode]').forEach(function (b) {
      /* the inline (i) lives inside this button — tapping it explains, it doesn't select */
      b.addEventListener('click', function (e) {
        if (e.target.closest('[data-info]')) return;   /* the inline (i) explains, it doesn't select */
    ...
    function applyPayGating() {
      ...
      ['hold', 'deposit', 'plan'].forEach(function (mode) {
        ...
        if (!b) return;
        /* aria-disabled rather than the disabled property: a disabled button swallows all
           pointer events, which would make the (i) tooltip unreachable exactly when a
           traveller most wants to know what the payment plan is. */
        b.setAttribute('aria-disabled', within60 ? 'true' : 'false');
